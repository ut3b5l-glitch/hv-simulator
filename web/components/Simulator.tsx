...
// Sampling the *entire* order (not just the top 3) lets us build a per-runner
// finishing-position distribution as well as win / top-3 / exotic rates.
interface SimResult {
  win: number[];
  top3: number[];
...
function simulate(runners: Runner[], nSims: number): SimResult {
  ...
  for (let s = 0; s < nSims; s++) {
    ...
    if (n > 1) top3[second]++;
    if (n > 2) top3[third]++;
    if (n > 1) exMap.set(first * n + second, (exMap.get(first * n + second) || 0) + 1);
    if (n > 2) {
  ...
  return {
    ...
    posDist: posCount.map((row) => row.map((c) => (c / nSims) * 100)),
    exacta,
    trifecta,
    nSims,
  };
...
export default function Simulator({ meetings }: { meetings: Meeting[] }) {
  ...
  const maxWin = Math.max(...runners.map((r) => r.win_pct), 1);
    ...
    <div className="space-y-4">
          ...
          return (
            <button
              ...
              className={`tap num shrink-0 rounded-pill px-3.5 py-1.5 text-caption font-semibold transition ${
                on ? "bg-white text-neutral-900" : "glass text-ink-60"
              }`}
            >
              {m.meeting_date}
            </button>
          ...
          return (
            <button
              ...
              className={`tap num rounded-tile py-2 text-callout font-bold transition ${
                on
                  ? "bg-gradient-to-br from-violet-400 to-indigo-500 text-white shadow-glow-indigo"
                  : "glass text-ink-60"
              }`}
            >
              R{r.race_number}
      ...
      <div className="flex items-center gap-2">
          ...
          {SIM_OPTIONS.map((opt) => (
            <button
              ...
              className={`tap num rounded-chip px-2.5 py-1.5 text-caption font-semibold transition ${
                nSims === opt ? "bg-white text-neutral-900" : "text-ink-70"
              }`}
            >
              {opt / 1000}k
            </button>
        ...
        <button
          ...
          className="tap flex-1 rounded-tile bg-gradient-to-r from-violet-400 to-indigo-500 py-2.5 text-body font-semibold text-white shadow-glow-indigo disabled:opacity-50"
        >
      ...
      {result && (
        <>
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="num shrink-0 text-right">
                    <div className="text-callout font-bold text-emerald-300">
                      {result.win[i].toFixed(1)}%
                    </div>
                    <div className="text-micro2 text-ink-80">win</div>
                  </div>
                </div>
                <div className="mt-2 grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-1.5">
                  <ProbBar value={result.win[i]} max={maxWin} tone="win" height={5} />
                  <span className="num w-16 text-right text-micro2 text-ink-80">
                    model {r.win_pct.toFixed(0)}%
                  </span>
                  <ProbBar value={result.top3[i]} tone="place" height={5} />
                  <span className="num w-16 text-right text-micro2 text-sky-300/80">
                    top-3 {result.top3[i].toFixed(0)}%
                  </span>
                </div>
              </div>
            ))}
          ...
          <FinishDistribution runners={runners} posDist={result.posDist} />
          ...
          <Exotic title="Most likely exacta · 1st–2nd">
            {result.exacta.map((e, k) => (
              <Combo
                ...
                pct={e.p}
                scale={2}
                names={[runners[e.a]?.horse_name, runners[e.b]?.horse_name]}
              />
            ))}
          </Exotic>
          <Exotic title="Most likely trifecta · 1st–2nd–3rd">
            {result.trifecta.map((t, k) => (
              <Combo
                ...
                scale={3}
                names={[
                  runners[t.a]?.horse_name,
                  runners[t.b]?.horse_name,
                  runners[t.c]?.horse_name,
                ]}
              />
            ))}
          </Exotic>
          ...
          <p className="px-2 pb-2 pt-1 text-center text-micro text-ink-80">
            {result.nSims.toLocaleString()} Plackett–Luce draws over the model&apos;s win
...
function Combo({
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <ProbBar value={pct * scale} tone="indigo" height={5} className="w-16 shrink-0" />
