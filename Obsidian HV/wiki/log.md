...
## [2026-06-27] query | Competitive positioning vs NowTV「AI神數」+ two /roast council runs → [[strategy/competitive-positioning-2026-06]]. Analysed HKT/NowTV's「AI神數」launch (AI + 玄學 dual-data racing pack, Master 侯天同, ~HK$138, csl-SIM bundle, NO accuracy claim) — validates the *category* but on a belief/personality/bundle axis Zokki can't match head-on. Roast #1 (market at $12/$48/$88): RESHAPE — kill the monthly edge tiers, lead with HK$12 + free, AI cards as cold-start loop. Roast #2 (Purple-Star 紫微斗數 pivot, fused HK$68): fused "chart picks your horses" = KILL (fixed chart can't map to race winners = noise; one-time chart ≠ recurring fee; Apple 5.3 + HK bookmaking risk; honest-vs-神棍 brand contradiction) → RESHAPE = SPLIT: racing stays the bilingual "honest-with-a-wink" anti-神棍 brand; 紫微斗數 engine becomes a SEPARATE recurring 運程 product; only overlap = a free for-fun "lucky-horse" share card. Counter-positioning plan + 48-hr tests + open handover decisions filed. No artifacts built (per user).

## [2026-07-05] experiment | Zokki GTM uplift — onboarding, AI analyst, landing rebuild
Ported Aurellar's (Stellar project) best patterns into Zokki. Consumer app: 4-act onboarding (punter-style quiz → race-night profile, localStorage-gated), OpenRouter AI stack with two surfaces — "Zokki AI Deep Dive" (per-race streaming briefing, grounded in server-built digest of our own model output) and "Ask Zokki" (grounded analyst chat, new nav tab), per-IP rate limits, responsible-gambling guardrails baked into the system prompt. Landing: Lenis smooth scroll, scroll-progress bar, magnetic CTAs, 3D floodlit-oval hero (R3F comet runners in butter/gold/green), pinned 4-stage phone showcase, rebuilt closing CTA; receipts refreshed to live numbers (67 races, top pick 51% vs fav 61% vs random 25% — published honestly). Both apps build clean.
